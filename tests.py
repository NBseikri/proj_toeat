...
from helper_functions import create_user, filter_trackings, sort_trackings, format_add, find_friends, suggest_friends, pending_friends, accept_new_friend, request_new_friend, delete_friend, create_tracking, get_all_trackings, get_match, tracking_cities
import os
...
class MyAppUnitTestCase(TestCase):
    ...
    def test_accept_new_friend(self):
        user = User.query.filter(User.username == "mgellar").first()
        user3 = User.query.filter(User.username == "pbuffet").first()
        friendship = Friend.query.filter_by(friend_one=user3.user_id, friend_two=user.user_id, status=2).first()
        self.assertTrue(accept_new_friend(user3.user_id, user.user_id), friendship)
    ...
    #     self.assertTrue(create_trackings_and_rests(user.user_id, "Mr. Holmes Bakehouse, Larkin Street, San Francisco, CA, United States", True, None, "Incredible pastries!"))

    def test_get_all_trackings(self):
        user = User.query.filter(User.username == "mgellar").first()
        restaurant = Restaurant.query.filter(Restaurant.rest_id==1).first()
        trackings = db.session.query(Tracking).filter_by(user_id=user.user_id, rest_id=restaurant.rest_id).all()
        self.assertTrue(get_all_trackings(user.user_id, restaurant.rest_id), trackings)

    def test_get_match(self):
        restaurant = Restaurant.query.filter_by(rest_id=1).first()        
        match = Restaurant.query.filter_by(rest_name=restaurant.rest_name).all()
        self.assertTrue(get_match(restaurant.rest_name), match)

    def test_tracking_cities(self):
        user = User.query.filter_by(user_id=1).first()
        cities = ['San Francisco', 'Seattle']
        self.assertTrue(tracking_cities(user.user_id), cities)
...
class FlaskTests(TestCase):
  ...
  def test_display_login_form(self):
    ...
    self.assertEqual(result.status_code, 200)
    self.assertIn('<h1>Log in to your To-eat List account</h1>', result.data)
...
#FAIL
  ...
  def test_display_registration_form(self):
    ...
    self.assertEqual(result.status_code, 200)
    self.assertIn('<label>First Name: </label>', result.data)
...
#RUNTIME ERROR
  def test_display_profile(self):
    user = User.query.filter(User.username == "mgellar").first()
    result = self.client.get("/profile/user.user_id")
    self.assertEqual(result.status_code, 200)
    self.assertIn('<h1>Welcome,' + user.first_name + '!</h1>', result.data)

#RUNTIME ERROR
  def test_process_add_rest(self):
    user = User.query.filter(User.username == "mgellar").first()
    result = self.client.get("/profile/user.user_id")
    self.assertEqual(result.status_code, 200)
